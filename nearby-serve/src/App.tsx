...
const App: React.FC = () => {
  ...
  // 🔴 REALTIME DATABASE LISTENERS (THIS FIXES MULTI PHONE SYNC)
  useEffect(() => {
    const unsubRecipients = firebaseService.listenRecipients(setRecipients);
    const unsubRequests = firebaseService.listenRequests(setRequests);
  ...
  const handleDonationRequest = async (
    reqData: Omit<DeliveryRequest, 'id' | 'status' | 'createdAt' | 'updatedAt' | 'recipientName' | 'pickupLocation'>
  ) => {
